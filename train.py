...
data_gen = DataGen('./data/' + dataset + '/', './data/' + dataset + '/train/', split_ratio=0.2, x=input_dim_x, y=input_dim_y)
...
######### MobilenetV2 ##########AK: only takes 224x224 images, UNet will be used instead
...
csv_logger = CSVLogger('./training_history/Benchmark3.csv', separator = ';')
...
save_history(model, model_name, training_history, dataset, n_filters, epochs, learning_rate, loss, color_space='RGB',
             path='./training_history/', temp_name = 'Benchmark3')