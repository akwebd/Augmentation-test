...
class DataGen:
    # AK: modified to add custom path_train
    # AK: modified to use validation dataset
    def __init__(self, path, path_train, split_ratio, x, y, color_space='rgb'):
        self.x = x
        self.y = y
        self.path = path
        ...
        self.path_train_images = path_train + "images/"
        self.path_train_labels = path_train + "labels/"
    ...
    def get_num_data_points(self, train=False, val=False, test=False):
        try:
            if train is True:
                image_file_list = self.x_train_file_list
            elif val is True:
    ...
    def change_color_space(image, label, color_space):
        if color_space.lower() in ['hsi', 'hsv']:
...
def save_history(model, model_name, training_history, dataset, n_filters, epoch, learning_rate, loss,
    ...
    save_weight_filename = temp_name if temp_name else str(datetime.datetime.now().isoformat(timespec='minutes'))
